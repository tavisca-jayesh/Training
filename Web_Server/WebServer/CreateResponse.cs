...
namespace WebServer
{
    class CreateResponse
    {
        ...
        {
            int dotPosition = requestedFile.LastIndexOf('.') + 1;
            if (dotPosition > 0)
            {
            ...
            else   //find default file as index .htm of index.html
            {
                if (FileHandler.FileExists("\\index.htm"))
                    SendResponse(ClientSocket, FileHandler.ReadFile("\\index.htm"), "200 Okay", "text/html");
                else if (FileHandler.FileExists("\\index.html"))
                    SendResponse(ClientSocket, FileHandler.ReadFile("\\index.html"), "200 Okay", "text/html");
                else
